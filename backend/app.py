...
FRONTEND_BUILD_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), os.pardir, "frontend", "build")
)
...
def get_indices():
    ...
    return jsonify(
        list(INDICES.keys())
    )  # Return only the display names for the dropdown
...
def compare_performance():
    ...
    try:
        print(
            f"Fetching data for Fund: {scheme_code}, Index: {index_symbol} ({index_name}) from {start_date_str} to {end_date_str}"
        )
        ...
        if isinstance(fund_result, Exception):
            return (
                jsonify(
                    {
                        "error": f"Error fetching fund data for {scheme_code}: {str(fund_result)}"
                    }
                ),
                500,
            )
        if fund_result is None:
            return (
                jsonify(
                    {
                        "error": f"No data found for fund {scheme_code} in the selected date range."
                    }
                ),
                404,
            )
        ...
        if isinstance(index_result, Exception):
            ...
            if "Connection refused" in str(
                index_result
            ) or "Network is unreachable" in str(index_result):
                error_detail = (
                    "Could not connect to NSE servers. Check network or firewall."
                )
            else:
                error_detail = str(
                    index_result
            ...
            return (
                jsonify(
                    {
                        "error": f"Error fetching index data for {index_symbol}: {error_detail}"
                    }
                ),
                500,
            )
        if index_result is None:
            return (
                jsonify(
                    {
                        "error": f"No data found for index {index_symbol} in the selected date range."
                    }
                ),
                404,
            )
        ...
        if performance_data.empty:
            return (
                jsonify(
                    {
                        "error": "Could not calculate performance. Check data availability for the selected period."
                    }
                ),
                500,
            )
        ...
        result = {
            ...
            "index_performance": performance_data["index_performance"]
            .round(2)
            .tolist(),
            "fund_actual_values": performance_data["fund_actual_values"]
            .round(2)
            .tolist(),
            "index_actual_values": performance_data["index_actual_values"]
            .round(2)
            .tolist(),
            ...
            "fund_name": next(
                (f["schemeName"] for f in ALL_FUNDS if f["schemeCode"] == scheme_code),
                scheme_code,
            ),
